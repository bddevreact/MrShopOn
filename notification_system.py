        ...
        try:
            ...
            for product in products:
                try:
                    ...
                    # Try to include product image if available
                    image_candidate = None
                    # Common product image keys: 'image', 'image_url', 'images'
                    if isinstance(product.get('images'), list) and product['images']:
                        image_candidate = product['images'][0]
                    elif product.get('image_url'):
                        image_candidate = product['image_url']
                    elif product.get('image'):
                        image_candidate = product['image']
                    ...
                    if image_candidate:
                        try:
                            if is_url(image_candidate):
                                ...
                                # Treat as local file path if exists
                                if os.path.exists(image_candidate):
                                    with open(image_candidate, 'rb') as photo_file:
                                        self.bot.send_photo(
                                            user_id,
                                            photo_file,
                                            caption=notification_text,
                            ...
                            # Fall back to text-only on any image error
                            print(f"Image send failed for product {product.get('name', 'Unknown')}: {e}")
                            sent = False
    ...
    def send_immediate_notification(self, user_id: int, product: Dict[str, Any] = None):
        ...
        if products:
            ...
            if isinstance(prod.get('images'), list) and prod['images']:
                image_candidate = prod['images'][0]
            elif prod.get('image_url'):
                image_candidate = prod['image_url']
            elif prod.get('image'):
                image_candidate = prod['image']
            ...
            if image_candidate:
                try:
                    ...
                    print(f"Immediate image send failed: {e}")
                    sent = False
